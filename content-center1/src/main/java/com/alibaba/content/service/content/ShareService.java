...
public class ShareService
{
	...
	public Share auditById(Integer id, ShareAuditDTO auditDTO)
	{
		...
		{
			//发送半消息
			this.rocketMQTemplate.sendMessageInTransaction("tx-add-bouns-group", "add-bouns", MessageBuilder.withPayload(UserAddBonusMsgDTO.builder().userId(share.getUserId()).bonus(50).build()).setHeader(RocketMQHeaders.TRANSACTION_ID, UUID.randomUUID().toString()).setHeader("share_id", id).build(), auditDTO);
		}
		else
		{
			// 2.进行审核操作
	...
	public void auditByIdWithRocketMqLog(Integer id, ShareAuditDTO auditDTO, String transactionId)
	{
		...
		//记录消息日志
		this.rocketmqTransactionLogMapper.insertSelective(RocketmqTransactionLog.builder().transactionId(transactionId).log("审核分享...").build());
	}
	/**
	 * Ribbon组成：
	 * IClientConfig:读取配置；
